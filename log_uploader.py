...
from log_parser import parse_log_content, summarise, LEVEL_COLORS
...
def _render_upload_zone() -> pd.DataFrame | None:
    ...
    with col_upload:
        uploaded = st.file_uploader(
            ...
            type=ACCEPTED_TYPES,
            help="Supports: Plain text, JSON (NDJSON), Syslog, Apache/Nginx, "
                 "Log4j, Python logging, Kubernetes, Windows Event Log, CSV/TSV",
...
def _render_summary_cards(stats: dict):
    ...
    card(c4, "📁", "Formats Found",
         len(stats.get("formats", [])), "#22c55e")
    ...
    if detected:
        badges = " · ".join(
            f"`{f}`" for f in detected if f not in ("unknown", "plaintext")
        )
        if badges:
...
def _render_charts(df: pd.DataFrame):
    ...
    with c_right:
        ...
        if not ts_df.empty:
            ...
            timeline = (
                ts_df.groupby(["minute", "level"]).size()
...
def _render_log_table(df: pd.DataFrame):
    ...
    with fc3:
        search_term  = st.text_input("🔎 Search in message / source",
                                     placeholder="e.g. NullPointerException, app.api",
                                     key="filter_search")

    # ── Apply filters ─────────────────────────
    mask = (
        df["level"].isin(sel_levels) &
...
def _render_rca_preview(df: pd.DataFrame):
    ...
    st.caption(
        f"Found **{len(errors)}** error-level entries. "
        "Connect to your RCA engine to run full analysis."
    )
...
def render_log_uploader():
    ...
    st.markdown(
        "Upload a log file in **any format** — the parser auto-detects the structure "
        "and normalises it for RCA analysis.",
        help="Supported: Plain text, JSON/NDJSON, Log4j, Syslog, Apache/Nginx, "
             "Kubernetes, Windows Event Log, Python logging, CSV/TSV",
    )
    st.divider()
