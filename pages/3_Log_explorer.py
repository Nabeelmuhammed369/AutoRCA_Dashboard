...
import pandas as pd
from log_source_manager import init_log_source, is_connected, get_log_df, get_source_label, get_source_meta
...
with fc3:
    ...
    sel_sources = st.multiselect("Source / Module", all_sources,
                                 default=all_sources, placeholder="All sources")

with fc4:
    search = st.text_input("🔎 Search message / source",
                           placeholder="e.g. NullPointerException, timeout, DB_CONN_FAIL")
...
if search:
    mask &= (
        df["message"].str.contains(search, case=False, na=False) |
...
with col_export:
    csv_out = filtered.drop(columns=["extra", "severity_rank", "level_icon",
                                      "is_error", "is_warning"], errors="ignore")
    st.download_button(
        "⬇️ Export CSV", csv_out.to_csv(index=False).encode(),
        "filtered_logs.csv", "text/csv", use_container_width=True,
    )
...
if not filtered.empty:
    with st.expander("📖 View full message for a specific row"):
        idx = st.number_input("Row #", min_value=0,
