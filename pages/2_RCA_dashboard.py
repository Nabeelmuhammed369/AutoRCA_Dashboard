...
from log_source_manager import (
    init_log_source, is_connected, get_log_df,
    get_log_stats, get_source_label, get_source_meta,
)
...
st.caption(
    f"Analysing: **{get_source_label()}** · "
    f"{meta.get('lines', 0):,} lines · "
...
with st.sidebar:
    ...
    if sim_db:
        ...
        if df is not None:
            ...
            fake_row = {
                "timestamp": pd.Timestamp.now(), "level": "ERROR",
                "source": "db.connector", "format": "standard",
                "message": "DB_CONN_FAIL: Connection refused by host 10.0.0.5:5432",
                "raw": "", "extra": {}, "severity_rank": 4,
                "level_icon": "🔴", "is_error": True, "is_warning": False,
            }
            st.session_state["log_source"]["df"] = pd.concat(
                [df, pd.DataFrame([fake_row])], ignore_index=True
            )
    ...
    if sim_api:
        ...
        if df is not None:
            ...
            fake_row = {
                "timestamp": pd.Timestamp.now(), "level": "ERROR",
                "source": "api.gateway", "format": "standard",
    ...
    if sim_mem:
        ...
        if df is not None:
            import pandas as pd
            fake_row = {
                "timestamp": pd.Timestamp.now(), "level": "WARNING",
                "source": "system.monitor", "format": "standard",
                "message": "Memory usage at 91% — GC pressure detected",
                "raw": "", "extra": {}, "severity_rank": 3,
                "level_icon": "🟡", "is_error": False, "is_warning": True,
            }
            st.session_state["log_source"]["df"] = pd.concat(
                [df, pd.DataFrame([fake_row])], ignore_index=True
            )
...
if run_diagnostic:
    ...
    with st.spinner("Running full diagnostic…"):
        ...
        if api_url:
            ...
            api_result = {"status_code": None, "response_time": None, "error": "No API URL configured"}
    ...
    st.session_state["api_history"].append(
        api_result.get("response_time") or 0
    )
    st.rerun()
...
# ─────────────────────────────────────────────
LEVEL_COLORS = {
    "CRITICAL": "#dc2626", "FATAL":   "#dc2626",
    "ERROR":    "#ef4444", "WARNING": "#f59e0b",
    "WARN":     "#f59e0b", "INFO":    "#22c55e",
    "DEBUG":    "#3b82f6", "TRACE":   "#94a3b8", "UNKNOWN": "#6b7280",
}
...
if not results:
    ...
    PLAYBOOK_STEPS = {
        "Data Integrity Issue":  ["1. Query DB for null/malformed records", "2. Check upstream ETL pipelines", "3. Enable strict input validation"],
        "Infrastructure Issue":  ["1. Check AWS / Azure status dashboard", "2. Review firewall & security groups", "3. Verify DNS and API Gateway config"],
        "Code Issue":            ["1. `git log --since='24h'` for recent changes", "2. Check error rate in affected service", "3. Roll back if critical"],
        "System Healthy":        ["✅ System is operating normally", "Continue scheduled monitoring"],
    }

    if "Healthy" in classification:
        st.markdown(f'<div class="ok-card">✅ <strong>{classification}</strong> — No issues detected.</div>', unsafe_allow_html=True)
