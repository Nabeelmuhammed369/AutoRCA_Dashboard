...
SEVERITY_RANK = {
    "FATAL": 6, "CRITICAL": 5, "ERROR": 4,
    "WARNING": 3, "WARN": 3, "INFO": 2,
    "DEBUG": 1, "TRACE": 0, "UNKNOWN": -1,
}
...
def _try_json(line: str) -> Optional[dict]:
    ...
    entry["extra"] = {k: v for k, v in obj.items()
                      if k not in ("timestamp","time","@timestamp","ts","date","datetime",
                                   "level","severity","log_level","lvl","loglevel",
                                   "message","msg","text","log","body",
                                   "logger","source","module","service","app","name")}
    return entry


def _try_syslog(line: str) -> Optional[dict]:
...
def _try_apache(line: str) -> Optional[dict]:
    ...
    entry["level"]   = ("ERROR" if status >= 500 else
    ...
    entry["extra"]   = {
        "ip": m.group("ip"), "status": status,
        "bytes": m.group("bytes"), "request": m.group("request"),
    }
    return entry


def _try_log4j(line: str) -> Optional[dict]:
...
def _try_standard(line: str) -> Optional[dict]:
...
def _try_k8s(line: str) -> Optional[dict]:
...
def _try_python_simple(line: str) -> Optional[dict]:
...
def _try_windows(line: str) -> Optional[dict]:
...
# CSV / TSV DETECTION
# ─────────────────────────────────────────────
def _try_parse_csv(content: str) -> Optional[pd.DataFrame]:
...
def _enrich(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["severity_rank"] = df["level"].map(
        lambda l: SEVERITY_RANK.get(str(l).upper(), -1)
    )
    df["level_icon"] = df["level"].map(
        lambda l: LEVEL_COLORS.get(str(l).upper(), "⚫")
    )
